...
def write_scoreboard(result: dict[str, Any], path: Path) -> str:
    ...
    lines = [
        ...
        "| Metric | Value |",
        "|---|---:|",
        f"| Query count | `{result['summary']['query_count']}` |",
        f"| Passed expectations | `{result['summary']['passed_expectations']} / {result['summary']['query_count']}` |",
        f"| Avg query wall | `{result['summary']['avg_query_wall_ms']} ms` |",
        f"| Avg router latency | `{result['summary']['avg_router_latency_ms']} ms` |",
        "",
    ...
    for row in result["queries"]:
        selected = ", ".join(row["summary"]["selected_ids"]) or "none"
        lines.append(
            f"| {row['label']} | {row['wall_ms']} | {row['summary']['router_latency_ms']} | "
            f"{row['summary']['packet_mode']} | `{selected}` | {row['passed_expectation']} |"
        )
...
def main() -> int:
    ...
    parser.add_argument("--scoreboard-path", type=Path, default=None)
    ...
    if args.scoreboard_path is not None:
        paths["scoreboard"] = write_scoreboard(result, args.scoreboard_path.resolve())
