...
def write_gate_report(gate: dict[str, Any], out: Path) -> None:
    ...
    if "external_generalization" in gate:
        ...
        if external.get("source_removal_ablation") is not None:
            removal = external["source_removal_ablation"]
            removal_latency = removal.get("latency_ms", {})
            lines.extend(
                [
                    f"| External source-removal pass | `{removal['passed']} / {removal['cases']}` |",
                    f"| External source-removal avg selected | `{removal['evidence_metrics']['avg_selected']}` |",
                    f"| External source-removal p95 latency | `{external['status']['source_removal_p95_latency_ms']} ms` |",
