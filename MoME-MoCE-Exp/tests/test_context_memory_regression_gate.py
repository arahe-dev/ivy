...
def sample_gate() -> dict:
    return {
        ...
        "plugin_benchmark": {"summary": {"passed_expectations": 6, "query_count": 6, "avg_router_latency_ms": 2.2}},
    }


def test_gate_status_requires_all_checks() -> None:
    gate = sample_gate()
    assert gate_status(gate, max_router_ms=5.0, max_plugin_router_ms=15.0)["passed"] is True

    gate["plugin_benchmark"]["summary"]["passed_expectations"] = 5
    status = gate_status(gate, max_router_ms=5.0, max_plugin_router_ms=15.0)
...
def test_write_gate_report_contains_core_sections(tmp_path: Path) -> None:
    gate = sample_gate()
    gate["status"] = gate_status(gate, max_router_ms=5.0, max_plugin_router_ms=15.0)
